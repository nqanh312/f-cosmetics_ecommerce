            ...
            <div class="w-full px-5">
                <c:if test="${carts.size() <= 0}">
                    <div class="w-full min-h-[50vh] flex justify-center items-center">
                        <p class="text-5xl">NO PRODUCT IN YOUR CART!</p>
                    </div>
                </c:if>
                <c:forEach items="${carts}" var="cart">
                    <div class="flex items-center relative bg-white rounded-lg border shadow-md w-full hover:bg-gray-100 mb-5">
                        <img style="width: 300px;" class="w-full h-full object-cover rounded-t-lg" src="${cart.product.images.get((cart.product.images.size()-(cart.product.images.size()-1))-1).image}" alt="">
                        <div class="flex-1">
                            ...
                            <div class="flex flex-col justify-between p-4 leading-normal">
                                ...
                                <div>
                                    ...
                                    <script>
                                        ...
                                        price_not_discount = price_not_discount.toLocaleString('en-US', {style: 'currency', currency: 'USD'});
                                        price_da_discount = price_da_discount.toLocaleString('en-US', {style: 'currency', currency: 'USD'});
                                        $("#tien-not-discount").text(price_not_discount);
                                        $("#tien-da-discount").text(price_da_discount);
                                        $("#tien-da-discount1").text(price_da_discount);
                                        }
                                        })
                                        } else {
            ...
            <div class="w-96">
                <div class="p-4 w-96 bg-white rounded-lg border border-gray-200 shadow-md sm:p-6 lg:p-8">
                    <div class="border-dashed border-b-2 pb-6 mb-4">
                        <h5 class="text-2xl font-semibold text-gray-900">Total</h5>
                        <!--<p class="text-lg font-medium mt-4">Quantity: ${quantity}</p>-->
                        <p class="text-lg font-medium mt-2">Total (not include discount): <span id="tien-not-discount">${totalPrice}</span></p>
                        <script>
                                    var tien = Number.parseInt($("#tien-not-discount").text());
                            tien = tien.toLocaleString('en-US', {style: 'currency', currency: 'USD'});
                            $("#tien-not-discount").text(tien);
                        </script>

                        <p class="text-lg font-medium mt-2">Total (include discount): <span id="tien-da-discount1">${realPrice}</span></p>
                        <script>
                            var tien = Number.parseFloat($("#tien-da-discount1").text());
                            tien = tien.toLocaleString('en-US', {style: 'currency', currency: 'USD'});
                            $("#tien-da-discount1").text(tien);
                        </script>
                        <p class="text-lg font-medium mt-2">Shipping fee: <span id="phi-ship">2</span></p>

                        <p class="text-lg font-medium mt-2">Total: <span id="tien-da-discount">${realPrice}</span></p>
                        <script>
                            var tienship = Number.parseFloat($("#phi-ship").text());
                            tienship = tienship.toLocaleString('en-US', {style: 'currency', currency: 'USD'});
                            $("#phi-ship").text(tienship);
                            var ship = Number.parseFloat($("#phi-ship"));
                            var tiendis = Number.parseFloat($("#tien-da-discount").text()) + 2;
//                            var tien = Number.parseFloat($("#tien-da-discount")) + 2;
//                            tien = tien.text();
                            tiendis = tiendis.toLocaleString('en-US', {style: 'currency', currency: 'USD'});
                            $("#tien-da-discount").text(tiendis);
                        </script>

                    </div>
                    <form class="space-y" action="/checkout" class="w-96 mt-5" method="post" id="form-checkout">
                        <h5 class="text-xl font-medium text-gray-900 mb-5">Customer information</h5>
                        ...
                        <c:if test="${carts.size() <= 0}">
                            <div class="w-full mt-4 text-white bg-gray-400 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Please add product to cart</div>    
                        </c:if>                          
                        <c:if test="${carts.size() > 0}">
                            <button type="submit" class="w-full mt-4 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Order now</button>    
                        </c:if>

                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            ...
                            <li aria-current="page">
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-500">Product</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                </div>
                <div class="w-full px-15 pt-5">
                    <div class="overflow-x-auto shadow-md sm:rounded-lg">
                        <div class="inline-block min-w-full align-middle">
                            <div class="flex items-center p-4">
                                <label for="table-search" class="sr-only">Search</label>
                                <form action="/admin/products" method="GET" class="flex items-center">
                                ...
                                <div class="ml-auto">
                                    <a href="/admin/products/add" class="inline-flex items-center py-2 px-4 text-sm font-medium text-center text-gray-900 bg-white rounded-lg border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-blue-300">Add products</a>
                                </div>
                            </div>
                                ...
                                <table class="min-w-full divide-y divide-gray-200 table-fixed">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200" id="tbodyUser">
                                        <c:forEach items="${products}" var="product">
                                            <tr class="hover:bg-gray-100" id="user-item-${product.getId()}">
                                                <!--                                                <td class="p-4 w-4">
                                                                                                    <div class="flex items-center">
                                                ...
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">${product.getId()}</td>
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900">${product.getName()}</td>
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">${product.getBrand()}</td>
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap" id="price-${product.getId()}">${product.getPrice()}
                                                    <script>
                                                        var price = Number.parseInt($("#price-${product.getId()}").text());
                                                        price = price.toLocaleString('en-US', {style: 'currency', currency: 'USD'});
                                                        $("#price-${product.getId()}").text(price);
                                                    </script>
                                                </td>
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">${product.getQuantity()}</td>
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">${product.isIsSale()}</td>
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">${product.getDiscount()}</td>
                                                <!--<td class="py-4 px-6 text-sm font-medium text-gray-900">${product.getDescription()}</td>-->
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">${product.getCategory().getName()}</td>
                                                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">${product.getState().getName()}</td>
                                                <td class="py-4 px-6 text-sm font-medium text-center whitespace-nowrap">
                                                    

                                                    <a href="/admin/products/detail?id=${product.getId()}" class="text-blue-600 hover:underline">View</a>
                                                    <a href="/admin/products/edit?id=${product.getId()}" class="ml-3 text-yellow-600 hover:underline">Edit</a>
                                                    <a onclick="return confirm('Are you sure you want to delete this product?');" href="/admin/products/delete?id=${product.getId()}" class="ml-3 text-red-600 hover:underline">Delete</a>
                ...
                <div class="mt-10 mb-5 w-full flex justify-center">
                    <nav aria-label="Page navigation example">
                        <ul class="inline-flex -space-x-px">
                            <li>
                                <a href="/admin/products?page=<%=pagination.getPrev()%>" data="<%=pagination.getPrev()%>" class="pagination-link py-2 px-3 ml-0 leading-tight text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a>
                            </li>
                            <c:if test="${pagination.getPageIndex()>2}">
                                <li>
                                    <a href="/admin/products?page=<%=pagination.getPageIndex() - 2%>" data="<%=pagination.getPageIndex() - 2%>" class="pagination-link py-2 px-3 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"><%=pagination.getPageIndex() - 2%></a>
                                </li>
                            </c:if>
                            <c:if test="${pagination.getPageIndex()>1}">
                                <li>
                                    <a href="/admin/products?page=<%=pagination.getPageIndex() - 1%>" data="<%=pagination.getPageIndex() - 1%>" class="pagination-link py-2 px-3 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"><%=pagination.getPageIndex() - 1%></a>
                                </li>
                            </c:if>
                            <li>
                                <a href="/admin/products?page=<%=pagination.getPageIndex()%>" data="<%=pagination.getPageIndex()%>" aria-current="page" class="pagination-link py-2 px-3 text-blue-600 bg-blue-50 border border-gray-300 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white"><%=pagination.getPageIndex()%></a>
                            </li>

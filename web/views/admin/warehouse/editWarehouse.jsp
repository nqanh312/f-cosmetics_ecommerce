                    ...
                    <div class="flex w-full">
                        <div>
                            <div class="flex justify-center items-center" >
                                <form action="/admin/warehouse/edit" method="post" style="min-width: 500px; height: 1000px;">
                                    ...
                                    <input type="hidden" value="${w.getId()}" name="importID">
                                    <div class="mb-6">
                                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Quantity</label>
                                        <input required type="number" id="quantity"  name="quantity" value="${w.productQuantity}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    </div>

                                    <div class="mb-6">
                                        <label for="price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">price</label>
                                        <input required type="number"  step="0.01" name="price" value="${w.price}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    </div>
                                    <div class="mb-6">
                                        <label for="quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Supplier</label>
                                        <input required type="text"  name="supplier" value="${w.supplier}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    </div>
                                    <div class="mb-6">
                                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400" >User</label>
                                        <select name="uid" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                            <c:forEach items="${users}" var="group">
                                                <option value="${group.getId()}" ${w.userId == group.getId() ?"checked":""}>${group.getUsername()}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                    <div class="mb-6">
                                        <label  class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">State</label>
                                        <input value="1" type="radio" name="state" ${w.state==1?"checked":""}> Active
                                        <input value="0" type="radio" name="state" ${w.state!=1?"checked":""}> Inactive<br>
                                    </div>
                                    <div class="mb-12">
                                        ...
                                        <select  onclick="Setlimit()" id="productSelect"  name="productID" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                            <c:forEach items="${products}" var="pro">
                                                <option value="${pro.getId()}-${pro.getQuantity()}" ${pro.getId() == w.getProductId() ?"checked":""}>${pro.getName()}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Edit</button>
                                </form>
                            </div>
                        </div>
                    </div>

...
public class UserDBContext extends DBContext<User> {
    ...
    public ArrayList<User> getUsersAdmin(int pageIndex, int pageSize) {
        ...
        String sql = "SELECT * FROM \n"
                ...
                + "WHERE [user].[permission] ='admin' and  row_index >= (? - 1) * ? + 1 AND row_index <= ? * ?";
    ...
    public User getUser(String username, String password) {
        String sql = "SELECT [user].[id]\n"
                ...
                + " WHERE ([user].[username] = ? or [user].[email] = ?) and [user].[password] = ? and [user].[is_active] = 1";
        PreparedStatement statement = null;
        try {
            statement = connection.prepareStatement(sql);

...
public class ProductDBContext extends DBContext<Product> {
    ...
    public ArrayList<Product> getProductsSort(String search, String arrange, int idGroup, int idCategory, int pageIndex, int pageSize) {
        ...
        String sql = "SELECT * FROM (SELECT [product].[id]\n"
                ...
                + " WHERE [product].[name] LIKE ? and [product].[isDelete] = 0";
        if (idGroup != -1) {
            sql += " AND [group].[id] = ? ";
            if (idCategory != -1) {
                sql += " AND [category].[id] = ? ";
        ...
        } else if (arrange.equals("Oldest")) {
            sql += "ORDER BY [product].[created_at]";
        } else if (arrange.equals("Newest")) {
        ...
        try {
            ...
            while (result.next()) {
                ...
                product.setIsDelete(result.getBoolean("isDelete"));
    ...
    public ArrayList<Product> getProducts(String search, int idGroup, int idCategory, int pageIndex, int pageSize) {
        ...
        try {
            ...
            while (result.next()) {
                ...
                product.setQuantity(result.getInt("quantity"));
                product.setDescription(result.getString("description"));
                product.setContent(result.getString("content"));
                product.setIsSale(result.getBoolean("isSale"));
                product.setIsDelete(result.getBoolean("isDelete"));
                product.setDiscount(result.getInt("discount"));
                product.setCreated_at(result.getTimestamp("created_at"));
    ...
    public ArrayList<Product> getProductsIndex(String search, int idGroup, int idCategory, int pageIndex, int pageSize) {
        ArrayList<Product> products = new ArrayList<>();
        ImageDBContext imageDB = new ImageDBContext();
        String sql = "SELECT * FROM (SELECT [product].[id]\n"
                + "      ,[product].[name]\n"
    ...
    public ArrayList<Product> findByCategory(int categoryId, int productId, int pageIndex, int pageSize) {
        ArrayList<Product> products = new ArrayList<>();
        ImageDBContext imageDB = new ImageDBContext();
        String sql = "SELECT * FROM (SELECT [product].[id]\n"
                + "      ,[product].[name]\n"
                ...
                + "  INNER JOIN [group] ON [group].[id] = [category].[groupId]\n"
                + "  INNER JOIN [state] ON [state].[id] = [product].[state]\n"
                + " WHERE [product].[categoryId] = ? AND [product].id != ?) [product]\n"
                + " WHERE row_index >= (? - 1) * ? + 1 AND row_index <= ? * ?";
        PreparedStatement statement = null;
        try {
            statement = connection.prepareStatement(sql);
    ...
    public ArrayList<Product> findByCategorySame(int categoryId, int productId, int pageIndex, int pageSize) {
        ArrayList<Product> products = new ArrayList<>();
        ImageDBContext imageDB = new ImageDBContext();
        String sql = "SELECT * FROM (SELECT [product].[id]\n"
                + "      ,[product].[name]\n"
                ...
                + "  INNER JOIN [state] ON [state].[id] = [product].[state]\n"
                + " WHERE [product].[categoryId] = ? AND [product].id != ?) [product]\n"
                + " WHERE row_index >= (? - 1) * ? + 1 AND row_index <= ? * ? AND [product].[isDelete] = 0";
        PreparedStatement statement = null;
        try {
            statement = connection.prepareStatement(sql);
            statement.setInt(1, productId);
    ...
    public Product insert(Product model) {
        ...
        try {
            ...
            statement.setBoolean(8, model.isIsDelete());
            statement.setInt(9, model.getDiscount());
            statement.setTimestamp(10, model.getCreated_at());
            statement.setTimestamp(11, model.getUpdated_at());
            statement.setInt(12, model.getCategoryId());
            statement.setInt(13, model.getStateId());
    ...
    public void update(Product model) {
        ...
        try {
            ...
            statement.setInt(12, model.getCategoryId());
            statement.setInt(13, model.getStateId());
            statement.setInt(14, model.getId());
            statement.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(ProductDBContext.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
